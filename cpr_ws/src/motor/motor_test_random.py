...
# ============================================================
# 테스트용 랜덤 압박 깊이 설정
USE_RANDOM_COMPRESSION_DEPTH = True

RANDOM_DEPTH_MIN_CM = 2.0
RANDOM_DEPTH_MAX_CM = 4.0
RANDOM_DEPTH_STEP_CM = 0.1
...
def get_travel_units():
    ...
    # 테스트용 랜덤 압박 깊이 모드
    # ==================================================
    if USE_RANDOM_COMPRESSION_DEPTH:
        min_step = int(RANDOM_DEPTH_MIN_CM / RANDOM_DEPTH_STEP_CM)
        max_step = int(RANDOM_DEPTH_MAX_CM / RANDOM_DEPTH_STEP_CM)

        depth_cm = random.randint(min_step, max_step) * RANDOM_DEPTH_STEP_CM
        ...
        travel_units = int(round(motor_rev * cfg.COMMAND_UNITS_PER_MOTOR_REV))

        print(f"\n[RANDOM INPUT] 압박 깊이 자동 설정: {depth_cm:.1f} cm")
        print(f"[RANDOM INPUT] travel_mm={travel_mm:.1f}")
        print(f"[RANDOM INPUT] travel_units={travel_units}")
